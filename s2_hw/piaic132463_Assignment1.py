...
def function2():
    ...
    x = np.arange(1,28,dtype=np.float64).reshape((3,3,3))     +9
...
def function5():
    ...
    z = np.where(np.arange(20).reshape(4,5)!=0,0,0)
...
def function6():
    ...
    arr = np.where(np.arange(10)!=0,0,0);arr[4]=10;arr[7]=20
    return arr
...
def function8():
    ...
    x = np.where(np.arange(10, dtype=np.uint32)!=6,6,6).reshape(2,5)
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.array([np.array(arr[x-1]-x) for x in brr])
...
def function18():
    ...
    x = len(np.where(np.isin(piaic,students))[0])
    return x
